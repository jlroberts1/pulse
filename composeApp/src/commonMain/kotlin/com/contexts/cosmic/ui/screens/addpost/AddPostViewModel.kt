...
import com.contexts.cosmic.ui.screens.addpost.composables.TenorGif
...
class AddPostViewModel(private val actorRepository: ActorRepository, private val tenorRepository: TenorRepository) : ViewModel() {
    ...
    suspend fun onGifSearchQueryChanged(newText: String) {
        delay(300)
        when (val response = tenorRepository.searchTenor(newText)) {
            is Response.Success -> {
                _uiState.update {
                    it.copy(gifSearchResults = response.data.results)
                }
            }
            is Response.Error -> {
                _uiState.update {
                    it.copy(error = response.error.message)
                }
            }
        }
    }
